...
function ensureOverlayDependencies(packageNames) {
  ...
  // Fix: remove nested @azure/msal-common inside msal-node (v16 breaks AccountEntity export)
  // so it falls back to our pinned v14 at the top level
  const nestedMsal = path.join(workspaceRoot, 'node_modules', '@azure', 'msal-node', 'node_modules', '@azure', 'msal-common');
  if (fs.existsSync(nestedMsal)) {
    fs.rmSync(nestedMsal, { recursive: true, force: true });
  }
...
function applyOpenClaudeClawPatches() {
  // Copy memory tool files from the source repo into the workspace
  const patchSource = path.resolve(versionRoot, '..', 'src', 'tools', 'MemoryTool');
  const patchDest = path.join(workspaceRoot, 'src', 'tools', 'MemoryTool');

  if (fs.existsSync(patchSource)) {
    fs.mkdirSync(patchDest, { recursive: true });
    for (const file of fs.readdirSync(patchSource)) {
      fs.copyFileSync(path.join(patchSource, file), path.join(patchDest, file));
    }

    // Create .js shims in node_modules/src so Bun resolves the imports
    const shimBase = path.join(workspaceRoot, 'node_modules', 'src', 'tools', 'MemoryTool');
    fs.mkdirSync(shimBase, { recursive: true });
    for (const name of ['MemorySearchTool', 'MemorySaveTool', 'prompt']) {
      const ext = name === 'prompt' ? '.ts' : '.ts';
      const shimContent = `import * as module_0 from "../../../../src/tools/MemoryTool/${name}${ext}";\nexport * from "../../../../src/tools/MemoryTool/${name}${ext}";\nexport default module_0.default;\n`;
      fs.writeFileSync(path.join(shimBase, `${name}.js`), shimContent);
    }

    // Patch tools.ts to import and register memory tools
    const toolsPath = path.join(workspaceRoot, 'src', 'tools.ts');
    let toolsContent = fs.readFileSync(toolsPath, 'utf8');

    if (!toolsContent.includes('MemorySearchTool')) {
      toolsContent = toolsContent.replace(
        "import { AskUserQuestionTool } from './tools/AskUserQuestionTool/AskUserQuestionTool.js'",
        "import { AskUserQuestionTool } from './tools/AskUserQuestionTool/AskUserQuestionTool.js'\nimport { MemorySearchTool } from './tools/MemoryTool/MemorySearchTool.js'\nimport { MemorySaveTool } from './tools/MemoryTool/MemorySaveTool.js'"
      );
      ...
      console.log('[OpenClaudeClaw] Patched tools.ts with MemorySearch + MemorySave');
    }
  } else {
    console.log('[OpenClaudeClaw] No memory tool source found at ' + patchSource + ', skipping');
  }
}
